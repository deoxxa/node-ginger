...
Compiler.prototype.make_ast = function(input) {
  var nodes = this.compile_nodes(input).filter(
...
Compiler.prototype.compile = function(input) {
  return uglify.gen_code(
    this.make_ast(input),
    {beautify: true, indent_start: 2, indent_level: 2});
};
...
Compiler.prototype.compile_nodes = function(nodes) {
  var self = this;
  return this.join_arrays(nodes.map(
    function(n) { return self.compile_node(n); }));
};
