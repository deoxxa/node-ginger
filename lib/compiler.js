...
Compiler.prototype.compile_expression = function(input, config, state) {
  ...
  switch (input.source.type) {
    ...
    case "list": { result = this.compile_list(input.source, config, state); break; }
  }
...
Compiler.prototype.compile_list = function(input, config, state) {
  var self = this;
  return ["array", input.data.map(function(node) { return self.compile_expression(node, config, state); })];
};
